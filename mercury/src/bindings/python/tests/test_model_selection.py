...
class TestEnumerateModels(unittest.TestCase):
    ...
    def test_enumerateModels(self):
        ...
        model_names = set(ManifestUtils.getModelName(entry.metadata) for entry in all_available_models)
...
class TestModelCollectionMethods(unittest.TestCase):
    ...
    def test_model_entries_properties(self):
        model_names = set(ManifestUtils.getModelName(entry.metadata) for entry in self.all_available_models)
        self.assertEqual(model_names, {'AlexNet', 'ChatGPT-cloud'})
        
    def test_select_all_match(self):
        filterElement = mc.Filter.fromArgs()
        
        model_names = set(ManifestUtils.getModelName(entry.metadata) for entry in self.all_available_models.select(filterElement))
        self.assertEqual(model_names, {'AlexNet', 'ChatGPT-cloud'})
